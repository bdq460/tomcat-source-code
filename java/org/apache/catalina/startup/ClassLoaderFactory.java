...
public final class ClassLoaderFactory {
    ...
    public static ClassLoader createClassLoader(List<Repository> repositories,
        ...
        //NOTE AccessController是一个本地类,应该是boot class loader记载的,其用于校验本段代码的执行者是否有权限访问所需访问资源
        //若可以访问则返回申请对象,否则抛出异常
        return AccessController.doPrivileged(
                new PrivilegedAction<StandardClassLoader>() {
                    @Override
    ...
    //NOTE 校验文件是否有效DIR与GLOB类型需要文件存在,并且是目录,并且可读,JAR文件需要文件存在且可读
    private static boolean validateFile(File file,
        ...
        if (RepositoryType.DIR == type || RepositoryType.GLOB == type) {
            if (!file.exists() || !file.isDirectory() || !file.canRead()) {
                ...
                //NOTE ${catalina.home}/lib是可选项,如果不存在仅debug输出，其他则warn输出
