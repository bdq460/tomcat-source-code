...
public class Catalina {
     ...
     */
    //NOTE 根据server.xml生成对象,进行组装,初始化,但不启动任何组件
    public void load() {

        long t1 = System.nanoTime();

        //NOTE 重新设置环境变量  CATALINA_HOME_PROP = "catalina.home" CATALINA_BASE_PROP = "catalina.base" 采用全路径进行设置
        initDirs();

        // Before digester - it may be needed
        //NOTE 初始化jndi名称服务环境变量
        //catalina.useNaming = true
        //java.naming.factory.url.pkgs = org.apache.naming:null
        ...
        // NOTE 若server.xml不存在则加载embed模式的配置文件
        if( inputStream==null ) {
            try {
                inputStream = getClass().getClassLoader()
